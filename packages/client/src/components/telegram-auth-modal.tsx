  ...
  useEffect(() => {
    ...
    (window as any).onTelegramAuth = async (user: any) => {
      ...
      try {
        ...
        // Widget should automatically change button to "Login as [Name]"
        // But sometimes it doesn't update, so we recreate the widget
        ...
        setTimeout(() => {
          ...
          if (container && botUsername) {
            ...
            clientLogger.info('Widget recreated, should show button with user name');
          }
        }, 500);
      } catch (error) {
        clientLogger.error('Telegram login error:', error);
